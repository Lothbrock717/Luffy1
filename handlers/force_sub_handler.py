...
async def handle_force_sub(bot: Client, cmd: Message):
    ...
    try:
        ...
        # Check using enum (works with all pyrogram/pyrofork versions)
        if user.status == enums.ChatMemberStatus.BANNED:
            ...
            return 400
        # User is a member, allow through
        return 200
    except UserNotParticipant:
        pass
    except Exception as err:
        print(f"Force sub error: {err}")
        return 200

    # User is not a participant — send join message
    try:
